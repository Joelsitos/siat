...
class FacturaOnline
{
	...
	public static function buildInvoice($codigoPuntoVenta = 0, $codigoSucursal = 0, $modalidad = 0,$dataFact,$fechaemision)
	{
		...
		while($datDetalle=mysqli_fetch_array($respDetalle)){
			...
			$detalle->actividadEconomica	= $dataFact['siat_codigoActividad'];
			$detalle->codigoProducto		= $codInterno;
			$detalle->codigoProductoSin		= $dataFact['siat_codigoProducto'];
		...
		// if($dataFact['siat_codigoActividad']==853000){//Para actividad educacion
		...
		// $solicitud->codigoDocumentoSector = 1; //instanciar
		// print_r($factura);
		return $factura;
	}
